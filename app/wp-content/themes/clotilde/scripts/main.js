(function($w) {
    ...
    var router = new CLO.Router();
    router.on('GO_TO', function(url) {
       navigationController.displayPage(url);
    });

    var navigationController = new CLO.NavigationController({
        el : $('#content')
    });

    // var $loader = $('#loader');
    // var $mainMenu = $('#site-navigation'),
    //     $mainMenuLink = $mainMenu.find('a');

    // var $menuActiveElement = $('<span>', {
    //     'class' : 'menu-active-element'
    // }).appendTo($mainMenu);

    // var menuElmt = new CLO.UI.MenuActiveElement({$wrapperElmt : $('#nav')});
    // $w.on('load', function() {
    //     menuElmt.create();
    //     resizableElmts.push(menuElmt);
    // });

    Backbone.history.start({
        pushState: true,
        root: CLO.CONFIG.PATH_URL
    });
    ...
    $('html').addClass(CLO.config.device);

    $('#nav_resp').on('click', function(e){
        e.preventDefault();
        e.stopPropagation();
        $(this).toggleClass('open');
        $('#menu_resp').toggleClass('show');
    })
    $('#menu_resp ul li a').on('click', function(){
        $('#nav_resp').toggleClass('open');
        $('#menu_resp').toggleClass('show');
    })

    $('html, body').on('mouseover','.category', function(){
        TweenLite.to($(this).find('.category-content-bg-visible'), 0.3, {opacity:'1', ease:Linear.ease});
    }).on('mouseout','.category', function(){
        TweenLite.to($(this).find('.category-content-bg-visible'), 0.3, {opacity:0, ease:Linear.ease});
    })
